    ...
    <div class="flex flex-wrap justify-between">
        ...
        <div class="bg-white rounded-[10px] shadow p-8 w-[530px]">
            ...
            <div class="mt-4 text-end">
                <button 
                    class="open-delete-drawer text-red-600 border border-red-500 hover:bg-red-50 font-semibold px-3 py-1.5 rounded text-xs"
                    data-id="{{ $karyawan->id }}"
                    data-name="{{ $karyawan->nama_depan }} {{ $karyawan->nama_belakang }}">
                    Hapus Akun
                </button>
            </div>
        </div>
...
document.addEventListener("DOMContentLoaded", function() {
    let deleteDrawer = document.getElementById("delete-drawer");
    let deleteServiceName = document.getElementById("delete-service-name");
    ...
    // Klik tombol hapus
    document.querySelectorAll(".open-delete-drawer").forEach(btn => {
        btn.addEventListener("click", function() {
            deleteId = this.getAttribute("data-id");
            deleteServiceName.textContent = this.getAttribute("data-name");
    ...
    deleteConfirmBtn.addEventListener("click", function() {
        if (deleteId) {
            ...
            .then(response => {
                if (response.ok) {
                    window.location.href = "{{ route('karyawan', ['role' => $karyawan->role]) }}";
                }
            });
        }
    });
