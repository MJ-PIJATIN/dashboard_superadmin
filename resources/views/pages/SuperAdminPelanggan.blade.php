        ...
        <div class="bg-white rounded-lg shadow-md p-6">
                    ...
                    <tbody>
                        @foreach ($customers as $index => $customer)
                            <tr class="hover:bg-gray-50 transition">
                                <td class="px-6 py-3 break-words w-[100px]">{{ $index + $customers->firstItem() }}</td>
                                <td class="px-6 py-3 break-words w-[240px]">
                                    <div class="flex items-center gap-2">
                                        <div class="flex items-center justify-center w-6 h-6 rounded-md 
                                            @if ($customer->gender == 'male') bg-blue-200 
                                            @elseif ($customer->gender == 'female') bg-pink-200 
                                            @endif">
                                ...
                                <td class="px-6 py-3 break-words w-[240px]">{{ $customer->email }}</td>
                                <td class="px-6 py-3 break-words w-[240px]">{{ $customer->city }}</td>
                                <td class="px-6 py-3 break-words w-[100px]">
                                    @php
                                        $isActive = $customer->status === 'Aktif';
                                    @endphp
                                    <form method="POST" action="{{ route('pelanggan.toggleStatus', ['id' => $customer->id]) }}">
                                        @csrf
                                        @method('PATCH')
                                        <button type="submit"
                                ...
                                <td class="px-6 py-3 break-words min-w-[100px]">
                                    <a href="{{ route('detail.akun.pelanggan', ['id' => $customer->id]) }}"
            ...
            @if ($customers->lastPage() > 1)
                <div class="flex justify-between items-center mt-4">
                    <span class="text-base font-regular text-gray-600">Halaman {{ $customers->currentPage() }} dari {{ $customers->lastPage() }}</span>
                    <div class="flex space-x-1 text-base font-semibold">
                        ...
                        @else
                            <a href="{{ $customers->previousPageUrl() }}" class="px-3 py-1 rounded bg-gray-200 hover:bg-gray-300">&lt;</a>
                        @endif

                        @foreach ($customers->getUrlRange(1, $customers->lastPage()) as $page => $url)
                            <a href="{{ $url }}" class="px-3 py-1 rounded {{ $page == $customers->currentPage() ? 'bg-teal-600 text-white' : 'bg-gray-200 hover:bg-gray-300' }}">{{ $page }}</a>
                        @endforeach

                        @if ($customers->hasMorePages())
                            <a href="{{ $customers->nextPageUrl() }}" class="px-3 py-1 rounded bg-gray-200 hover:bg-gray-300">&gt;</a>
                        @else
                            <button class="px-3 py-1 rounded bg-gray-200 text-gray-500" disabled>&gt;</button>
                        @endif
                    </div>
